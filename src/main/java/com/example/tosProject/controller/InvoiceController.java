...
import com.example.tosProject.service.ExcelWorker;
...
public class InvoiceController {
    ...
    public String aktProducts(@PathVariable("id")Long id, Model model, Principal principal){
        ...
        model.addAttribute("amounts", invoice.getAmount());
        model.addAttribute("user", userService.getUserByPrincipal(principal));
    ...
    public String addProductsForInvoice(@RequestParam ("productTitle") String productTitle,
        ...
        if(invoice.getAmount()==null) {
            ArrayList<Integer> amm = new ArrayList<>();
            invoice.setAmount(amm);
        }
    ...
    public String print(@PathVariable("id")Long id){
        Invoice invoice=invoiceService.findById(id);
        ExcelWorker.excelPrinter(invoice);
        return "redirect:/invoices";
    }
